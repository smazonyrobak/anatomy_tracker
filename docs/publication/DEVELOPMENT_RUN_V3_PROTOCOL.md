...
Scientific quality is **NOT ESTABLISHED**. No internal-development prediction, calibration, benchmark, external-validation, or final-test result was produced. On all 24 applied updates the model-selected truth-in-top-k fraction was zero while refinement was truth-forced at one, so honest coarse retrieval has not yet been demonstrated. The next gate is a new receipt-bound bounded finite S=9 pilot followed by prediction on every animal-disjoint internal-development row, with raw predictions, physical pose/deformation errors, honest-retrieval and failure accounting, and explicitly uncalibrated uncertainty diagnostics. Exact evidence is recorded in `arbitrary_plane_finite_v4_smoke_result.yaml`.
