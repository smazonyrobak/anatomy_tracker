...
The tracked DeepSlice comparison cannot support a paired superiority conclusion. Audit found that the Allen rasters were supplied in the opposite horizontal view from DeepSlice's intended A-to-P convention. A physical atlas-axis reflection made aggregate numbers appear plausible but reverses laterality and is not a valid repair. The correct rerun preserves raw bytes and raw ground truth, applies one deterministic horizontal image reparameterization before DeepSlice, back-transforms a `W`-pixel final O/U/V plane with `O'=O+((W-1)/W)U, U'=-U`, and then scores it with the separately versioned official `x/W,y/H` metric. The frozen historical `(O+U,-U,V)` adapter and `+0.5` metric remain diagnostic-only. Until that harmonized rerun is complete, the old DeepSlice AP/L--R/D--V deltas, confidence intervals and joint probability are invalid. The tracked `release_approved=false` and `promotion_ready=false` conclusions remain conservative.
