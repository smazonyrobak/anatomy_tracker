...
def run_automatic_warp_batch(
    ...
    for sequence, job in enumerate(jobs):
        if cancel_event.is_set():
            raise InterruptedError
        progress_messages.put(
            (sequence, f"Preparing {job['name']} ({sequence + 1} / {len(jobs)})...")
        )
        path = Path(job["path"])
        raw = (
            tifffile.imread(str(path))
        ...
        raw_slice, _ = transform_slice_image(
            normalize_u8(display_raw),
            job["rotation_deg"],
            job["flip_horizontal"],
            job["flip_vertical"],
        ...
        if len(job["surface_points"]) >= 8:
            base_h, _ = fit_surface_scale_translation(
                base_h, job["surface_points"], atlas_mask
            )
        canvases = dense_registration_canvases(
            raw_slice, slice_mask, atlas_image, atlas_mask, base_h
        )
        if cancel_event.is_set():
            raise InterruptedError
        progress_messages.put(
            (sequence, f"Warping {job['name']} ({sequence + 1} / {len(jobs)})...")
        )
        result = run_dense_registration(
            model_path,
            *canvases,
        ...
        completed.append(
            {
                ...
                "slice_image_sha256": array_sha256(raw_slice),
                "tissue_mask_sha256": array_sha256(slice_mask.astype(np.uint8)),
                "result": result,
            }
        )
        progress_messages.put(
            (sequence + 1, f"Warped {sequence + 1} / {len(jobs)} aligned slices")
        )
    return completed
...
def signed_probe_roll_deg(direction_ap_dv_ml: np.ndarray) -> float:
    """Signed horizontal bearing from the bregma-to-lambda axis."""
    direction = np.asarray(direction_ap_dv_ml, dtype=np.float64)
    horizontal = float(np.hypot(direction[0], direction[2]))
    if horizontal < 1e-9:
        return float("nan")
    roll = float(np.degrees(np.arctan2(direction[2], -direction[0])))
    if roll > 90.0:
        roll -= 180.0
    elif roll < -90.0:
        roll += 180.0
...
class TrajectoryTrackerWindow(QtWidgets.QMainWindow):
    def __init__(
        ...
        *,
        ...
        self._probe_fit_cache: dict[tuple, object] = {}
    ...
    def _build_ui(self, default_run_folder: str | Path) -> None:
        ...
        self.transform_btn = QtWidgets.QPushButton("Apply landmark warp")
        ...
        self.automatic_warp_all_btn = QtWidgets.QPushButton("Apply to all aligned slices")
        self.automatic_warp_all_btn.setToolTip(
            "Apply the trained anatomical warp to every non-stale slice produced by automatic section matching. "
            "Slices are processed sequentially in the background and installed together when the batch succeeds."
        )
        ...
        manual_layout.addWidget(self.automatic_warp_btn, 2, 0)
        manual_layout.addWidget(self.automatic_warp_all_btn, 2, 1)
        ...
        self.use_probe_constraints = QtWidgets.QCheckBox("Use surgical constraints")
        self.use_probe_constraints.setToolTip(
            ...
            "angle/depth limits. The final displayed trajectory is also fitted inside these hard bounds. "
            "Rerun automatic section matching after changes to update slice pose."
        )
        ...
        self.probe_constraint_scope = QtWidgets.QLabel(
            ...
            "3. Rerun automatic section matching to update slice pose. The trajectory fit updates immediately."
        )
        ...
        depth_help = (
            "Reject slice poses and trajectory fits that would place a marked "
            "electrode point deeper than this distance from the cortical entry. Leave off to use only "
            "the selected probe's physical shank limit. Rerun automatic section matching to update slice pose."
        )
    ...
    def _probe_constraints_changed(self, *_: object) -> None:
        ...
        self._update_probe_fit_summary()
        self.status.setText(
            f"{probe_name} trajectory was refitted to the surgical constraint. "
            "Rerun automatic section matching to apply the same constraint to slice pose."
        )
        self._refresh_3d()
    ...
    def _update_probe_fit_summary(self, *_: object) -> None:
        ...
        roll = signed_probe_roll_deg(stereo_deep_direction)
        ...
        if constraint is not None and constraint.enabled:
            ...
            lifecycle = (
                " | surgical constraint applied to alignment and trajectory"
                if applied
                else " | trajectory constrained; rerun section matching to constrain slice pose"
            )
        else:
            lifecycle = ""
        self.probe_fit_summary.setText(
            f"Observed fit: AP {stereo_entry[0]:+.0f} um | ML {stereo_entry[2]:+.0f} um | "
            f"angle {angle:.1f} deg | roll {roll:+.1f} deg | deepest mark {depth:.0f} um | {len(points)} points"
            + (
    ...
    def _clear_coordinate_registration(self, session: SliceSession) -> None:
        self._probe_fit_cache.clear()
        session.transformed_overlay = None
    ...
    def _invalidate_transform_after_landmark_edit(self, session: SliceSession) -> None:
        ...
        stale_reasons = [
            reason
            for reason in diagnostics.get("stale_reasons", [])
            if reason != "contributor transform landmarks changed"
        ]
        if stale_reasons:
            diagnostics["stale_reasons"] = stale_reasons
        else:
    ...
    def _refresh_point_counts(self) -> None:
        ...
        if session is None:
            ...
            self.automatic_warp_all_btn.setEnabled(False)
            self.auto_align_btn.setEnabled(False)
            self.auto_align_all_btn.setEnabled(False)
        ...
        self.automatic_warp_all_btn.setEnabled(
            len(self._automatic_warp_sessions()) >= 2
            and self.atlas_volume is not None
            and self.annotation_volume is not None
            and not self.auto_alignment_busy
        )
    ...
    def _automatic_warp_batch_snapshot(self, session: SliceSession) -> tuple:
        ...
        selection_digest = (
            ...
            else array_sha256(session.brain_brush_selection_mask.astype(np.uint8))
        )
        return (
            id(session),
            str(source),
            ...
            selection_digest,
            session.auto_alignment_run_id,
            bool((session.auto_alignment_diagnostics or {}).get("alignment_run_stale", False)),
            id(session.slice_atlas_transform),
            id(session.slice_to_atlas_tps),
            id(session.atlas_to_slice_tps),
        )
    ...
    def _automatic_warp_all_clicked(self) -> None:
        try:
            self.apply_automatic_anatomical_warp_all()
        except Exception as exc:
            QtWidgets.QMessageBox.critical(self, "Automatic anatomical warp failed", str(exc))
            self.status.setText(f"Automatic anatomical warp failed: {exc}")
    ...
    def apply_automatic_anatomical_warp_all(self) -> None:
        if self.auto_alignment_busy:
            return
        aligned = self._automatic_warp_sessions()
        if len(aligned) < 2:
            QtWidgets.QMessageBox.warning(
                self,
                "Aligned slices required",
                "Run automatic section matching on at least two non-stale slices first.",
            )
            return
        if self.atlas_volume is None or self.annotation_volume is None:
            return
        ...
        snapshots = {
            index: self._automatic_warp_batch_snapshot(session)
            for index, session in aligned
        }
        session_objects = {index: session for index, session in aligned}
        jobs = [
            {
                "session_index": index,
                "name": session.name,
                "path": session.path,
                "rotation_deg": float(session.rotation_deg),
                "flip_horizontal": bool(session.flip_horizontal),
                "flip_vertical": bool(session.flip_vertical),
                "surface_points": self._slice_raw_to_display_points(
                    session, session.brain_outline_points
                ),
                "outline_closed": bool(session.brain_outline_closed),
                "selection_mask": (
                    None
                    if session.brain_brush_selection_mask is None
                ...
                "atlas_index": int(session.atlas_index),
                "tilt_ml_deg": float(session.atlas_tilt_ml_deg),
                "tilt_dv_deg": float(session.atlas_tilt_dv_deg),
            }
            for index, session in aligned
        ]
        ...
        cancel_event = threading.Event()
        progress = QtWidgets.QProgressDialog(
            f"Warping {len(jobs)} aligned slices...",
            "Cancel",
            0,
            len(jobs),
            self,
        ...
        self._refresh_point_counts()
        self.status.setText(
            f"Automatically warping {len(jobs)} aligned slices; the interface remains available."
        )
        future = self.alignment_executor.submit(
            run_automatic_warp_batch,
            jobs,
            self.atlas_volume,
            self.annotation_volume,
            DENSE_REGISTRATION_MODEL_PATH,
            DENSE_REGISTRATION_METADATA_PATH,
            DENSE_REGISTRATION_MODEL_SHA256,
            DENSE_REGISTRATION_METADATA_SHA256,
            messages,
            cancel_event,
        )
        ...
        def install(results: list[dict]) -> None:
            current_atlas = (
                ...
                id(self.annotation_volume),
                str(self.atlas_folder),
                None if self.atlas_volume is None else tuple(self.atlas_volume.shape),
                tuple(sorted(self.atlas_file_hashes.items())),
                tuple(float(value) for value in self.bregma_voxel),
            )
            if current_atlas != atlas_snapshot:
                raise RuntimeError(
                    "The atlas changed while automatic anatomical registration was running; results discarded"
                )
            if {result["session_index"] for result in results} != set(snapshots):
                raise RuntimeError("Automatic anatomical registration returned an incomplete slice batch")
            for index, expected in snapshots.items():
                if (
                    index >= len(self.sessions)
                    or self.sessions[index] is not session_objects[index]
                    or self._automatic_warp_batch_snapshot(self.sessions[index]) != expected
                ):
                    raise RuntimeError(
                        f"{session_objects[index].name} changed while registration was running; all results discarded"
                    )
            staged = [
                self._stage_automatic_anatomical_warp_result(
                    self.sessions[result["session_index"]],
                    result["base_h"],
                    result["display_shape"],
                    result["slice_image_sha256"],
                    result["tissue_mask_sha256"],
                    result["result"],
                )
                for result in results
            ]
            self._commit_automatic_anatomical_warp_results(staged)
            self.status.setText(
                f"Applied automatic anatomical warp to {len(staged)} aligned slices; every probe observation was remapped."
            )
    ...
    def _apply_automatic_anatomical_warp_result(
        ...
        staged = self._stage_automatic_anatomical_warp_result(
            session,
            base_h,
            display_shape,
            slice_image_sha256,
    ...
    def _stage_automatic_anatomical_warp_result(
        self,
        session: SliceSession,
        base_h: np.ndarray,
        display_shape: tuple[int, int],
    ...
    def _commit_automatic_anatomical_warp_results(self, staged: list[dict]) -> None:
        for update in staged:
            session = update["session"]
            session.slice_atlas_transform = update["transform"]
            session.slice_to_atlas_tps = None
            session.atlas_to_slice_tps = None
            session.transformed_overlay = update["overlay"]
            session.auto_alignment_diagnostics = update["diagnostics"]
            self._commit_probe_coordinate_updates(session, update["probe_updates"])
        self._probe_fit_cache.clear()
        if any(update["session"] is self.current_session() for update in staged):
            self._refresh_atlas()
            self._refresh_points()
        self._update_probe_fit_summary()
        self._refresh_3d()
    ...
    def _start_auto_alignment(self, session_indices: list[int], *, global_alignment: bool) -> None:
        ...
        progress.setWindowTitle("Automatic section matching")
        progress.setWindowModality(QtCore.Qt.WindowModality.NonModal)
        progress.setMinimumDuration(0)
        progress.setAutoClose(False)
        progress.setAutoReset(False)
    ...
    def _apply_auto_alignment_results(
        ...
        *,
        ...
        if not prepared:
            raise RuntimeError("Automatic section matching returned no slices")
        batch_names = [
        ...
        if global_alignment:
            ...
            self.status.setText(
                ...
                f"{review_text} Add corresponding landmarks to apply the final nonlinear coordinate warp."
            )
        else:
    ...
    def probe_brain_geometry(self, probe_name: str) -> tuple[np.ndarray, np.ndarray, np.ndarray] | tuple[None, None, None]:
        ...
        constraint = self._probe_constraint(probe_name)
        if constraint is not None and constraint.enabled:
            observations = self.probe_observations_by_slice(probe_name)
            effective = self._effective_probe_constraint(constraint)
            cache_key = (
                probe_name,
                effective,
                id(self.annotation_volume),
                tuple(self.bregma_voxel),
                tuple(
                    (index, np.asarray(values, dtype=np.float64).tobytes())
                    for index, values in observations.items()
                ),
            )
            fit = self._probe_fit_cache.get(cache_key)
            if fit is None:
                fit = fit_probe_ray(
                    observations,
                    effective,
                ...
                if len(self._probe_fit_cache) >= 32:
                    self._probe_fit_cache.clear()
                self._probe_fit_cache[cache_key] = fit
            if fit is None:
                return None, None, None
            stereo_points = volume_to_stereotaxic_um(points, self.bregma_voxel)
            axial = (stereo_points - fit.entry_ap_dv_ml_um) @ fit.direction_ap_dv_ml
            maximum_depth = float(effective.maximum_insertion_depth_um)
            deepest_distance = float(np.clip(np.max(axial), 0.0, maximum_depth))
            deepest_stereo = (
                fit.entry_ap_dv_ml_um + fit.direction_ap_dv_ml * deepest_distance
            )
            entry = probe_stereotaxic_to_volume(
                fit.entry_ap_dv_ml_um, self.bregma_voxel, VOXEL_UM
            )
            deep_endpoint = probe_stereotaxic_to_volume(
                deepest_stereo, self.bregma_voxel, VOXEL_UM
            )
            surface_direction = -fit.direction_ap_dv_ml * STEREOTAXIC_AXIS_SIGN_AP_DV_ML
    ...
    def _write_manifest(
        ...
        stereo_deep_direction = -np.asarray(surface_direction) * STEREOTAXIC_AXIS_SIGN_AP_DV_ML
        stereo_deep_direction /= np.linalg.norm(stereo_deep_direction)
        attack_angle = float(
            np.degrees(np.arcsin(np.clip(abs(stereo_deep_direction[1]), 0.0, 1.0)))
        )
        roll = signed_probe_roll_deg(stereo_deep_direction)
        constraint = self._probe_constraint(probe_name)
        manifest = {
            ...
            "observed_probe_fit_uses_surgical_constraint": bool(
                constraint is not None and constraint.enabled
            ),
            "probe_attack_angle_convention": "0 degrees horizontal; 90 degrees vertical",
            "fitted_probe_attack_angle_deg": attack_angle,
            "fitted_probe_roll_deg": None if not np.isfinite(roll) else roll,
            "probe_roll_convention": (
                "signed horizontal bearing relative to the bregma-to-lambda AP axis; "
                "0 degrees parallel and +/-90 degrees perpendicular"
            ),
            ...
            "trajectory_sampling": "constrained best-fit shank centerline at each recording contact",
            "vertical_reference": (
