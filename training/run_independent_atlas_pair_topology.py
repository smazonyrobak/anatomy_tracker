"""Run the frozen paired native-versus-random-adjacency topology diagnostic."""
...
PURPOSE = "development-only-paired-native-adjacency-causal-diagnostic"
ROLE = "causal-rescue-premise-not-model-selection"
FAMILY = "independent-oracle-atlas-pair-topology-pair-1500-r1804322-v1"
RESUME_FORMAT = "independent-atlas-pair-topology-paired-state-v1"
FINAL_FORMAT = "independent-atlas-pair-topology-paired-final-v1"
FREEZE_FORMAT = "independent-atlas-pair-topology-joint-freeze-v1"
RECEIPT_FORMAT = "independent-atlas-pair-topology-paired-receipt-v1"
QUALIFICATION_SEEDS = (2104322, 2204322)
CONSUMED_OR_FORBIDDEN_SEEDS = (
    1004322,
    1104322,
    1204322,
    1304322,
...
CONSUMED_QUALIFICATION_SEEDS = CONSUMED_OR_FORBIDDEN_SEEDS
TRUTH_AP_MARGIN_UM = 500.0
TRUTH_TILT_ABS_MAX_DEG = 25.0
_QUALIFICATION_CAPABILITY = object()
PERMUTATION_SEEDS = {"8": 180432208, "16": 180432216}
PERMUTATION_RAW_SHA256 = {
    "8": "283089a1c7ff350cb32b15218cca8c8507a7ab6bf2d5cb739b7a93f52e078a9f",
    "inverse8": "100761938c9b19320414075ddfe1367848ab377fba7395e2caf6f60d9e0406b9",
    "16": "4cc5dd17a8328d0b66b4fb54cea2cc4ddd52f0d5f93e26ac9179128ec78a5617",
    "inverse16": "121716ff0e162330646b6b6c78d99bd5e5284db971d3ca3d7f7983c7e895d5a2",
}
OFF_CENTER_COEFFICIENT_COUNT = 1536
OFF_CENTER_PARAMETER_NAMES = tuple(
    f"topology{level}.blocks.{block}.depthwise.weight"
    for level in (8, 16)
    for block in range(3)
)
ATOMIC_REPLACE_DELAYS_SECONDS = (0.05, 0.10, 0.20, 0.40, 0.80, 1.0, 1.0, 1.0)
ATOMIC_REPLACE_ATTEMPTS = 1 + len(ATOMIC_REPLACE_DELAYS_SECONDS)
...
CONSUMED_SOURCE_SHA256 = {
    ...
    "training/synthetic_registration.py": (
        "5a7274b56cdfa95fdb410b1b441325c4ee9ab3b15eb8076be2781e92c00c1dde"
    ),
    "training/quicknii_plane_metric.py": (
        "fd3f3bc0a1f2e0b57b33db706c2d5d73134b3525bdc6989dfb4bcf2defd6dc31"
    ),
    "source/dense_registration_preprocessing.py": (
        "cad4a5fdbaa4be638abe6dbaf1add2c95c74bb10d4e2d851b2a8ea8682cf4f60"
    ),
}
SOURCE_FILES = (
    "training/independent_atlas_pair_topology.py",
...
def _determinism_state(device: torch.device) -> dict:
    return {
...
def configure_deterministic_execution(device: torch.device | None = None) -> dict:
    """Seal deterministic execution before any CUDA model or tensor operation."""
    workspace = os.environ.get("CUBLAS_WORKSPACE_CONFIG")
    if workspace is None:
        os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"
    elif workspace != ":4096:8":
        raise RuntimeError("CUBLAS_WORKSPACE_CONFIG differs from the frozen value")
    torch.use_deterministic_algorithms(True)
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False
    observed = _determinism_state(device or torch.device("cpu"))
    if observed != {
        "deterministic_algorithms": True,
        "cudnn_deterministic": True,
        "cudnn_benchmark": False,
        "cublas_workspace_config": ":4096:8",
        "cuda_execution": (device or torch.device("cpu")).type == "cuda",
    }:
        raise RuntimeError("deterministic execution contract was not established")
    return observed


def _assert_deterministic_execution(device: torch.device) -> dict:
    observed = _determinism_state(device)
    expected = {
        "deterministic_algorithms": True,
        "cudnn_deterministic": True,
        "cudnn_benchmark": False,
    ...
    if observed != expected:
        raise RuntimeError("deterministic execution contract changed")
    return observed


def permutation_contract() -> dict:
    tensors = {
        "8": PERMUTATION8,
        "inverse8": INVERSE_PERMUTATION8,
        "16": PERMUTATION16,
        "inverse16": INVERSE_PERMUTATION16,
    }
    observed_hashes = {name: _raw_tensor_sha256(value) for name, value in tensors.items()}
    inverse_checks = {
        "8": lattice_permutation_audit(
            torch.arange(COST_VOLUME_CHANNELS * 20 * 29).reshape(
                1, COST_VOLUME_CHANNELS, 20, 29
            ),
            PERMUTATION8,
            INVERSE_PERMUTATION8,
        ),
        "16": lattice_permutation_audit(
            torch.arange(COST_VOLUME_CHANNELS * 10 * 15).reshape(
                1, COST_VOLUME_CHANNELS, 10, 15
            ),
            PERMUTATION16,
            INVERSE_PERMUTATION16,
        ),
    ...
    retained_edges = {}
    for level, height, width, inverse in (
        ("8", 20, 29, INVERSE_PERMUTATION8),
        ("16", 10, 15, INVERSE_PERMUTATION16),
    ):
        edges = []
        for row in range(height):
            for column in range(width):
                ...
                if row + 1 < height:
                    edges.append((source, source + width))
        retained_edges[level] = {
            "retained": sum(
                abs(divmod(int(inverse[first]), width)[0] - divmod(int(inverse[second]), width)[0])
                + abs(divmod(int(inverse[first]), width)[1] - divmod(int(inverse[second]), width)[1])
                == 1
                for first, second in edges
            ),
            "native_total": len(edges),
        }
    payload = {
        "seeds": PERMUTATION_SEEDS,
        "lattice_shapes": {"8": [20, 29], "16": [10, 15]},
        "channels": COST_VOLUME_CHANNELS,
        "raw_tensor_sha256": observed_hashes,
        "fixed_points": {
            "8": int((PERMUTATION8 == torch.arange(len(PERMUTATION8))).sum()),
            "16": int((PERMUTATION16 == torch.arange(len(PERMUTATION16))).sum()),
        },
        "native_four_neighbor_edges_retained": retained_edges,
        "bijection_and_inverse_exact": inverse_checks,
    }
    if (
        observed_hashes != PERMUTATION_RAW_SHA256
        or payload["fixed_points"] != {"8": 2, "16": 1}
        or retained_edges
        != {
            "8": {"retained": 7, "native_total": 1111},
            "16": {"retained": 5, "native_total": 275},
        }
        or not all(
        all(checks.values()) for checks in inverse_checks.values()
        )
    ):
        raise RuntimeError("fixed random-adjacency permutation contract differs")
    payload["permutation_contract_sha256"] = _canonical_sha256(payload)
    return payload
...
def _replace_with_retry(temporary: Path, path: Path, expected_sha256: str) -> None:
    for attempt in range(ATOMIC_REPLACE_ATTEMPTS):
        try:
            os.replace(temporary, path)
            break
        except OSError as error:
            if not _retryable_replace_error(error) or attempt == ATOMIC_REPLACE_ATTEMPTS - 1:
                raise
            time.sleep(ATOMIC_REPLACE_DELAYS_SECONDS[attempt])
    if _sha256(path) != expected_sha256:
        raise RuntimeError("atomic replacement bytes differ from the immutable temporary")
...
def _atomic_json(path: Path, payload) -> None:
    ...
    if temporary.exists():
        raise RuntimeError(f"stale atomic temporary exists: {temporary.name}")
    payload_bytes = _canonical_bytes(payload) + b"\n"
    with temporary.open("xb") as stream:
        stream.write(payload_bytes)
        stream.flush()
        os.fsync(stream.fileno())
    _replace_with_retry(temporary, path, hashlib.sha256(payload_bytes).hexdigest())
...
def _atomic_torch(path: Path, payload) -> None:
    ...
    _replace_with_retry(temporary, path, expected_sha256)
...
def _assert_no_stale_temporaries(run_folder: Path) -> None:
    stale_temporaries = sorted(run_folder.glob("*.tmp"))
    if stale_temporaries:
        raise RuntimeError(
            "stale atomic temporaries require manual audit before startup: "
            + ", ".join(path.name for path in stale_temporaries)
        )
...
def _expected_config() -> dict:
    return {
        ...
        "family": {
            ...
            "null": {
                "class": "training.independent_atlas_pair_topology.AtlasPairEnergyScrambledTopologyControl",
                "adjacency": "fixed-random-lattice-permutation-before-identical-spatial-encoder-then-exact-inverse-restore",
            },
            "treatment": {
                "class": "training.independent_atlas_pair_topology.AtlasPairEnergyNativeTopology",
                "adjacency": "native-rectangular-cost-volume-lattice",
            },
            ...
            "claim_scope": "causal-native-rectangular-adjacency-versus-fixed-random-adjacency-only",
            "graph_matching": "identical-learned-and-trainable-graph;-null-adds-only-fixed-parameter-free-permutation-and-inverse",
            "null_is_not_topology_free": True,
            "permutation_contract": permutation_contract(),
        },
        "model": {
            "expected_parameter_count_each": 284058,
            ...
            "cost_volume_channels": 82,
            "topology_channels": 32,
            "topology_output_channels": 16,
            "topology_blocks": "three-depthwise-separable-residual-blocks-dilations-1-2-4",
            "statistics_dimension_each_level": 194,
            "head": "LayerNorm(194)-Linear(194,48)-GELU-Linear(48,1)",
        },
        "data": {
            ...
            "train_seed": 1904322,
            "train_count": 2048,
            "development_seed": 2004322,
            "development_count": 48,
            "qualification_seeds": [2104322, 2204322],
            "consumed_or_forbidden_prior_seeds": list(CONSUMED_OR_FORBIDDEN_SEEDS),
        ...
        "training": {
            ...
            "atomic_replace": {
                "attempts": 9,
                "delays_seconds": [0.05, 0.10, 0.20, 0.40, 0.80, 1.0, 1.0, 1.0],
                "retryable_errors": "Windows-5-or-32-and-EACCES-EPERM-EBUSY-only",
                "write_and_fsync_temporary_once": True,
                "preserve_temporary_on_exhaustion": True,
            },
            "checkpoint_reader_protocol": "no-concurrent-readers-of-pt-tmp-final-or-development-artifacts-until-runner-exits;monitor-stdout-and-gpu-telemetry-only",
            "progress_protocol": "flush-one-canonical-json-line-after-each-persisted-checkpoint",
        ...
        "integrity": {
            "complete_initial_state_and_outputs_bit_exact": True,
            "independent_state_storage": True,
            "permutations_bijective_fixed_and_hash_bound": True,
            "pixel_vector_multiset_and_inverse_recovery_exact": True,
            "off_center_coefficients_each": 1536,
            "off_center_initial_exact_zero": True,
            "off_center_gradient_every_update": "six-tensors-present-finite-positive-norm",
            "first_update_off_center_gradients": "arm-aggregate-hashes-differ-no-per-scalar-gate",
            "early_off_center_gradient_coverage": "cumulative-all-1536-by-completed-update-2-per-arm",
            "early_gradient_coverage_window_updates": 2,
            "update2_and_final_off_center_weights": "all-finite-all-nonzero-and-arm-hashes-differ",
            "enforce_topology_gradient_audit": True,
        ...
        "interpretation": {
            ...
            "both_fail": "change-feature-or-candidate-construction-before-recurrence",
            "treatment_pass_null_fail_paired_pass": "native-adjacency-evidence-authorize-independent-confirmation",
            "both_pass": "native-adjacency-necessity-not-shown",
            "one_seed_pass": "family-fail",
            "ranking_causal_pass_but_treatment_free_search_fail": "local-native-adjacency-supported-end-to-end-no-go",
            "prohibitions": [
                "no-early-stop",
                "no-extension",
                "no-gate-change",
...
def inspect_config(path: str | Path) -> dict:
    ...
    if raw != _expected_config():
        raise ValueError("paired topology config differs from the frozen family")
    if commitment != _canonical_sha256(raw):
        raise ValueError("paired topology family self-hash differs")
...
def initialize_pair(config: dict, device: torch.device):
    determinism = _assert_deterministic_execution(device)
    _seed_everything(int(config["optimizer_seed"]), device)
    null, treatment = make_atlas_pair_topology_pair()
    null = null.to(device)
    treatment = treatment.to(device)
    if parameter_count(null) != EXPECTED_PARAMETER_COUNT or parameter_count(
        treatment
    ) != EXPECTED_PARAMETER_COUNT:
        raise RuntimeError("paired topology parameter count changed")
    ...
    independent_storage = all(
        null_state[name].data_ptr() != treatment_state[name].data_ptr()
        for name in null_state
    )
    within_arm_storage_unique = {
        arm: len({value.data_ptr() for value in state.values()}) == len(state)
        for arm, state in (("null", null_state), ("treatment", treatment_state))
    }
    if not independent_storage or not all(within_arm_storage_unique.values()):
        raise RuntimeError("paired model states do not have independent storage")
    observed_permutation_contract = permutation_contract()
    expected_permutation_contract = config["family"]["permutation_contract"]
    if observed_permutation_contract != expected_permutation_contract:
        raise RuntimeError("paired model permutation contract differs from config")
    return {"null": null, "treatment": treatment}, {
        ...
        "independent_state_storage": independent_storage,
        "within_arm_state_storage_unique": within_arm_storage_unique,
        "permutation_contract_sha256": observed_permutation_contract[
...
def initial_optimizer_scaler_integrity(
    optimizers: dict, scalers: dict, config: dict, amp: bool
) -> dict:
    optimizer_states = {
        name: optimizer.state_dict() for name, optimizer in optimizers.items()
    }
    scaler_states = {name: scaler.state_dict() for name, scaler in scalers.items()}
    result = {
        "optimizer_states_empty": all(
            not value["state"] for value in optimizer_states.values()
        ),
        "optimizer_states_equal": _canonical_sha256(optimizer_states["null"])
        == _canonical_sha256(optimizer_states["treatment"]),
        "scaler_states_equal": scaler_states["null"] == scaler_states["treatment"],
        "scaler_scale": float(scalers["null"].get_scale()),
        "amp_enabled": amp,
    }
    expected_scale = (
        float(config["training"]["amp_initial_scale"]) if amp else 1.0
    )
    result["passed"] = bool(
        result["optimizer_states_empty"]
        and result["optimizer_states_equal"]
        and result["scaler_states_equal"]
        and result["scaler_scale"] == expected_scale
    )
    result["integrity_sha256"] = _canonical_sha256(result)
    if not result["passed"]:
        raise RuntimeError("paired optimizer/scaler initialization differs")
    return result


def _initialization_receipt_valid(initialization: dict, config: dict) -> bool:
    expected_initialization_keys = {
        ...
        "full_initial_state_equal",
        "independent_state_storage",
        "within_arm_state_storage_unique",
        "permutation_contract_sha256",
        "determinism",
        "optimizer_scaler_integrity",
        "initialization_sha256",
    }
    expected_optimizer_scaler_keys = {
    ...
    initialization_core = {
        ...
        if name != "initialization_sha256"
    }
    optimizer_scaler = initialization.get("optimizer_scaler_integrity", {})
    optimizer_scaler_core = {
        name: value
        for name, value in optimizer_scaler.items()
        if name != "integrity_sha256"
    }
    return bool(
        set(initialization) == expected_initialization_keys
        and initialization.get("initialization_sha256")
        == _canonical_sha256(initialization_core)
        and initialization.get("seed") == config["optimizer_seed"]
        and initialization.get("parameter_count_each") == EXPECTED_PARAMETER_COUNT
        and initialization.get("state_schema_sha256")
        == _canonical_sha256(initialization.get("state_schema", []))
        and initialization.get("null_state_sha256")
        == initialization.get("treatment_state_sha256")
        and initialization.get("full_initial_state_equal") is True
        and initialization.get("independent_state_storage") is True
        and initialization.get("within_arm_state_storage_unique")
        == {"null": True, "treatment": True}
        and initialization.get("permutation_contract_sha256")
        == config["family"]["permutation_contract"]["permutation_contract_sha256"]
        and initialization.get("determinism")
        == {
            "deterministic_algorithms": True,
            "cudnn_deterministic": True,
            "cudnn_benchmark": False,
            "cublas_workspace_config": ":4096:8",
            "cuda_execution": bool(config["training"]["amp"]),
        }
        and optimizer_scaler.get("passed") is True
        and set(optimizer_scaler) == expected_optimizer_scaler_keys
        and optimizer_scaler.get("optimizer_states_empty") is True
        and optimizer_scaler.get("optimizer_states_equal") is True
        and optimizer_scaler.get("scaler_states_equal") is True
        and optimizer_scaler.get("amp_enabled") is config["training"]["amp"]
        and optimizer_scaler.get("scaler_scale")
        == (
            float(config["training"]["amp_initial_scale"])
            if config["training"]["amp"]
            else 1.0
        )
        and optimizer_scaler.get("integrity_sha256")
        == _canonical_sha256(optimizer_scaler_core)
    )


def bind_optimizer_scaler_initialization(
    initialization: dict,
    optimizers: dict,
    scalers: dict,
    config: dict,
    ...
    result["optimizer_scaler_integrity"] = initial_optimizer_scaler_integrity(
        optimizers, scalers, config, amp
    )
    result["initialization_sha256"] = _canonical_sha256(result)
    if not _initialization_receipt_valid(result, config):
        raise RuntimeError("paired initialization receipt failed self-audit")
    return result
...
def write_manifest_receipt(
    ...
    frozen_sources = _assert_frozen_files(config)
    payload = {
        "format": "independent-atlas-pair-topology-manifest-v1",
        "purpose": PURPOSE,
        "role": role,
        "family_self_sha256": config["family_self_sha256"],
        "source_sha256": frozen_sources,
        "determinism": _assert_deterministic_execution(generator.device),
        "config_contract_sha256": config["contract_sha256"],
        "config_file_sha256": config["config_file_sha256"],
        "generator_contract_sha256": generator.contract["contract_sha256"],
        "permutation_contract_sha256": config["family"]["permutation_contract"][
            "permutation_contract_sha256"
        ],
        "manifest": manifest,
    }
...
def _topology_lattices_for_model(model, batch: dict) -> dict[str, torch.Tensor]:
    ...
    for level, source, atlas in (
        ...
        correlation = _local_correlation(source, atlas, model.radius)
        mask = F.interpolate(
            atlas_mask.float(), correlation.shape[-2:], mode="nearest"
        ).to(correlation)
        result[level] = torch.cat((correlation, mask), dim=1)
    return result


def initial_topology_integrity(
    models: dict, batch: dict, config: dict | None = None
) -> dict:
    ...
    with torch.no_grad():
        lattices = {
            name: _topology_lattices_for_model(model, batch)
            for name, model in models.items()
        }
        outputs = {
            name: model(
                ...
                candidate_chunk_size=(
                    config["model"]["candidate_chunk_size"] if config else 8
                ),
            )
            for name, model in models.items()
        }
    ...
    permutation_by_level = {
        "8": (models["null"].permutation8, models["null"].inverse_permutation8),
        "16": (models["null"].permutation16, models["null"].inverse_permutation16),
    }
    levels = {}
    for level in ("8", "16"):
        native = lattices["treatment"][level]
        null_native = lattices["null"][level]
        permutation, inverse = permutation_by_level[level]
        scrambled = permute_lattice(null_native, permutation)
        audit = lattice_permutation_audit(null_native, permutation, inverse)
        levels[level] = {
            "native_lattice_exact_across_arms": bool(torch.equal(null_native, native)),
            "channels": int(null_native.shape[1]),
            "pixel_vector_count": int(null_native.shape[-2] * null_native.shape[-1]),
            "permutation_bijection": audit["bijection"],
            "pixel_vector_multiset_exact": audit["vector_multiset_exact"],
            "inverse_recovery_exact": audit["recovery_exact"],
            "scrambled_exact_index_select": bool(
                torch.equal(
                    scrambled.flatten(2),
                    null_native.flatten(2).index_select(2, permutation),
                )
            ),
            "native_lattice_sha256": _tensor_sha256(native),
            "scrambled_lattice_sha256": _tensor_sha256(scrambled),
        }
    output_exact = all(
        torch.equal(outputs["null"][name], outputs["treatment"][name])
        for name in ("energy", "energy8", "energy16")
    )
    off_center = _off_center_audit(models, gradients=False)
    state = {name: model.state_dict() for name, model in models.items()}
    complete_state_exact = list(state["null"]) == list(state["treatment"]) and all(
        torch.equal(state["null"][key], state["treatment"][key])
        for key in state["null"]
    )
    independent_storage = all(
        state["null"][key].data_ptr() != state["treatment"][key].data_ptr()
        for key in state["null"]
    )
    within_arm_storage_unique = {
        arm: len({value.data_ptr() for value in arm_state.values()})
        == len(arm_state)
        for arm, arm_state in state.items()
    }
    contract = permutation_contract()
    result = {
        "levels": levels,
        "complete_initial_state_exact": complete_state_exact,
        "complete_initial_outputs_exact": output_exact,
        "initial_output_sha256": {
            name: {key: _tensor_sha256(value) for key, value in output.items()}
            for name, output in outputs.items()
        },
        "independent_state_storage": independent_storage,
        "within_arm_state_storage_unique": within_arm_storage_unique,
        "permutation_contract_sha256": contract["permutation_contract_sha256"],
        "permutations_bijective_fixed_and_hash_bound": all(
            value["permutation_bijection"] for value in levels.values()
        ),
        "pixel_vector_multiset_and_inverse_recovery_exact": all(
            value["pixel_vector_multiset_exact"]
            and value["inverse_recovery_exact"]
            and value["scrambled_exact_index_select"]
            and value["native_lattice_exact_across_arms"]
            and value["channels"] == COST_VOLUME_CHANNELS
            for value in levels.values()
        ),
        "off_center_initial": {
            name: {
                **value,
                "exact_zero": value["nonzero_count"] == 0,
            }
            for name, value in off_center.items()
        },
    }
    result["integrity_sha256"] = _canonical_sha256(result)
    if not (
        result["complete_initial_state_exact"]
        and result["complete_initial_outputs_exact"]
        and result["independent_state_storage"]
        and all(result["within_arm_state_storage_unique"].values())
        and result["permutations_bijective_fixed_and_hash_bound"]
        and result["pixel_vector_multiset_and_inverse_recovery_exact"]
        and all(
            value["count"] == OFF_CENTER_COEFFICIENT_COUNT
            and value["finite"]
            and value["exact_zero"]
            for value in result["off_center_initial"].values()
        )
    ):
        raise RuntimeError("paired topology initialization integrity failed")
    return result


def _topology_integrity_valid(payload: dict, config: dict) -> bool:
    expected_keys = {
        "levels",
        "complete_initial_state_exact",
        "complete_initial_outputs_exact",
        "initial_output_sha256",
        "independent_state_storage",
        "within_arm_state_storage_unique",
        "permutation_contract_sha256",
        "permutations_bijective_fixed_and_hash_bound",
        "pixel_vector_multiset_and_inverse_recovery_exact",
        "off_center_initial",
        "integrity_sha256",
    }
    commitment = payload.get("integrity_sha256")
    core = {name: value for name, value in payload.items() if name != "integrity_sha256"}
    return bool(
        set(payload) == expected_keys
        and commitment == _canonical_sha256(core)
        and payload.get("complete_initial_state_exact") is True
        and payload.get("complete_initial_outputs_exact") is True
        and payload.get("independent_state_storage") is True
        and payload.get("within_arm_state_storage_unique")
        == {"null": True, "treatment": True}
        and payload.get("permutations_bijective_fixed_and_hash_bound") is True
        and payload.get("pixel_vector_multiset_and_inverse_recovery_exact") is True
        and payload.get("permutation_contract_sha256")
        == config["family"]["permutation_contract"]["permutation_contract_sha256"]
        and set(payload.get("off_center_initial", {})) == {"null", "treatment"}
        and all(
            value.get("count") == OFF_CENTER_COEFFICIENT_COUNT
            and value.get("finite") is True
            and value.get("exact_zero") is True
            and len(value.get("per_tensor", {})) == 6
            and all(
                tensor.get("count") == 256
                and tensor.get("finite") is True
                and tensor.get("nonzero_count") == 0
                for tensor in value.get("per_tensor", {}).values()
            )
            and _off_center_audit_value_valid(
                {name: item for name, item in value.items() if name != "exact_zero"},
                include_indices=False,
                require_each_positive=False,
            )
            for value in payload.get("off_center_initial", {}).values()
        )
    )
...
def _off_center_audit(
    models: dict, *, gradients: bool, include_indices: bool = False
) -> dict:
    result = {}
    expected_names = set(OFF_CENTER_PARAMETER_NAMES)
    for arm, model in models.items():
        ...
        for name, parameter in model.named_parameters():
            ...
            if value is None:
                raise RuntimeError(f"off-centre tensor is unavailable: {arm}:{name}")
            mask = torch.ones(3, 3, dtype=torch.bool, device=value.device)
            mask[1, 1] = False
            tensors[name] = value.detach()[..., mask].reshape(-1)
        if set(tensors) != expected_names:
            raise RuntimeError(f"off-centre tensor schema differs for {arm}")
        aggregate = torch.cat([tensors[name] for name in sorted(tensors)])
        result[arm] = {
            "count": int(aggregate.numel()),
            "finite": bool(torch.isfinite(aggregate).all()),
            "nonzero_count": int(torch.count_nonzero(aggregate)),
            "positive_norm": bool(torch.linalg.vector_norm(aggregate) > 0),
            "sha256": _tensor_sha256(aggregate),
            **(
                {
                    "nonzero_indices": torch.nonzero(
                        aggregate, as_tuple=False
                    )[:, 0]
                    .detach()
                    .cpu()
                    .tolist()
            ...
            "per_tensor": {
                name: {
                    ...
                    "positive_norm": bool(torch.linalg.vector_norm(value) > 0),
...
def _off_center_audit_value_valid(
    value: dict, *, include_indices: bool, require_each_positive: bool
) -> bool:
    expected_keys = {
        "count",
        "finite",
    ...
    if include_indices:
        expected_keys.add("nonzero_indices")
    return bool(
        set(value) == expected_keys
        and value.get("count") == OFF_CENTER_COEFFICIENT_COUNT
        and value.get("finite") is True
        and 0 <= value.get("nonzero_count", -1) <= OFF_CENTER_COEFFICIENT_COUNT
        and isinstance(value.get("positive_norm"), bool)
        and isinstance(value.get("sha256"), str)
        and len(value["sha256"]) == 64
        and set(value.get("per_tensor", {})) == set(OFF_CENTER_PARAMETER_NAMES)
        and all(
            set(tensor) == {
                "count",
                "finite",
            ...
            and tensor.get("count") == 256
            and tensor.get("finite") is True
            and 0 <= tensor.get("nonzero_count", -1) <= 256
            and isinstance(tensor.get("positive_norm"), bool)
            and (not require_each_positive or tensor["positive_norm"] is True)
            and isinstance(tensor.get("sha256"), str)
            and len(tensor["sha256"]) == 64
            for tensor in value["per_tensor"].values()
        )
    )
...
def paired_optimizer_update(
    ...
    enforce_topology = config["integrity"].get(
        "enforce_topology_gradient_audit", True
    )
    if enforce_topology:
        early_window = int(
            config["integrity"]["early_gradient_coverage_window_updates"]
        )
        topology_gradients = _off_center_audit(
            models,
            gradients=True,
            include_indices=max(step_before.values()) < early_window,
        )
        if not all(
            value["count"] == OFF_CENTER_COEFFICIENT_COUNT
            and value["finite"]
            and value["positive_norm"]
            and len(value["per_tensor"]) == 6
            and all(
                tensor["count"] == 256
                and tensor["finite"]
                and tensor["positive_norm"]
                for tensor in value["per_tensor"].values()
            )
            for value in topology_gradients.values()
        ):
            raise RuntimeError("off-centre topology gradients failed the paired barrier")
        if set(step_before.values()) == {0} and (
            topology_gradients["null"]["sha256"]
            == topology_gradients["treatment"]["sha256"]
        ):
            raise RuntimeError("first-update off-centre topology gradients differ from contract")
    ...
    topology_weights = _off_center_audit(models, gradients=False) if enforce_topology else None
    for name in models:
    ...
    if enforce_topology and set(step_after.values()) == {2} and (
        any(
            value["count"] != OFF_CENTER_COEFFICIENT_COUNT
            or not value["finite"]
            or value["nonzero_count"] != OFF_CENTER_COEFFICIENT_COUNT
            for value in topology_weights.values()
        )
        or topology_weights["null"]["sha256"]
        == topology_weights["treatment"]["sha256"]
    ):
        raise RuntimeError("update-2 off-centre topology weights differ from contract")
    ...
    return {
        ...
        "off_center_gradient": topology_gradients,
        "off_center_weight_after": topology_weights,
    }


def _early_topology_coverage_valid(
    history: list[dict], update: int, config: dict
) -> bool:
    if not config["integrity"].get("enforce_topology_gradient_audit", True):
        return True
    window = int(config["integrity"]["early_gradient_coverage_window_updates"])
    if window != 2 or len(history) != update:
        return False
    for item, row in enumerate(history):
        gradients = row.get("off_center_gradient", {})
        for arm in ("null", "treatment"):
            indices = gradients.get(arm, {}).get("nonzero_indices")
            if item < window:
                if (
                    not isinstance(indices, list)
                    or indices != sorted(set(indices))
                    or len(indices) != gradients[arm].get("nonzero_count")
                    or any(index < 0 or index >= OFF_CENTER_COEFFICIENT_COUNT for index in indices)
                ):
                    return False
            elif indices is not None:
                return False
    if history and (
        history[0]["off_center_gradient"]["null"]["sha256"]
        == history[0]["off_center_gradient"]["treatment"]["sha256"]
    ):
        return False
    if update >= window:
        for arm in ("null", "treatment"):
            ...
            if covered != set(range(OFF_CENTER_COEFFICIENT_COUNT)):
                return False
        weights = history[window - 1].get("off_center_weight_after", {})
        if (
            set(weights) != {"null", "treatment"}
            or any(
                not value.get("finite")
                or value.get("count") != OFF_CENTER_COEFFICIENT_COUNT
                or value.get("nonzero_count") != OFF_CENTER_COEFFICIENT_COUNT
                for value in weights.values()
            )
            or weights["null"].get("sha256") == weights["treatment"].get("sha256")
        ):
            return False
    return True
...
def _history_is_valid(history: list[dict], update: int, config: dict) -> bool:
    ...
    return (
        ...
        and all(
            (
                set(value.get("off_center_gradient", {}))
                == {"null", "treatment"}
                and all(
                    _off_center_audit_value_valid(
                        row,
                        include_indices=item
                        < config["integrity"][
                            "early_gradient_coverage_window_updates"
                        ],
                        require_each_positive=True,
                    )
                    for row in value.get("off_center_gradient", {}).values()
                )
                and set(value.get("off_center_weight_after", {}))
                == {"null", "treatment"}
                and all(
                    _off_center_audit_value_valid(
                        row,
                    ...
                    for row in value["off_center_weight_after"].values()
                )
            )
            if config["integrity"].get("enforce_topology_gradient_audit", True)
            else value.get("off_center_gradient") is None
            and value.get("off_center_weight_after") is None
            for item, value in enumerate(history)
        )
        and _early_topology_coverage_valid(history, update, config)
    )
...
def _resume_payload(
    ...
    if config["integrity"].get(
        "enforce_topology_gradient_audit", True
    ) and (
        not _topology_integrity_valid(topology_integrity, config)
        or not _initialization_receipt_valid(initialization, config)
    ):
        raise RuntimeError("paired topology initialization integrity differs")
    ...
    return {
        ...
        "initial_topology_integrity": topology_integrity,
        "update": update,
...
def _load_joint_resume(
    ...
    if (
        ...
        or (
            config["integrity"].get("enforce_topology_gradient_audit", True)
            and not _topology_integrity_valid(
                state.get("initial_topology_integrity", {}), config
            )
        )
    ):
    ...
    return (
        ...
        state["initial_topology_integrity"],
    )
...
def _absolute_arm_status(
    ...
    checks = {
        ...
        "order_equivariance": (
            ...
            and order["decoded_pose_maximum_difference"] == 0.0
...
def paired_panel_status(
    ...
    if not all(integrity_checks.values()):
        ...
        branch = "both-pass-native-adjacency-necessity-not-shown"
    elif (
        arm_status["treatment"]["passed"]
        and not arm_status["null"]["passed"]
        and causal_passed
    ):
        branch = "native-adjacency-evidence-authorize-independent-confirmation"
    elif (
        causal_passed
        and treatment_fixed_status["passed"]
        and not arm_status["treatment"]["passed"]
    ):
        branch = "local-native-adjacency-supported-end-to-end-no-go"
    elif not arm_status["null"]["passed"] and not arm_status["treatment"]["passed"]:
        branch = "both-fail-change-feature-or-candidate-construction-before-recurrence"
    else:
...
def _training_integrity(
    ...
    final_model_states_differ: bool = True,
) -> dict:
    ...
    checks = {
        "history_schema_valid": _history_is_valid(history, len(history), config)
        and len(history) == maximum,
        "full_initial_state_equal": initialization.get("full_initial_state_equal")
        is True,
        "independent_initial_state_storage": (
            initialization.get("independent_state_storage") is True
            if enforce_topology
            else True
        ),
        "optimizer_scaler_initialization": (
            _initialization_receipt_valid(initialization, config)
            if enforce_topology
            else True
        ),
        "initial_topology_integrity": (
            _topology_integrity_valid(topology_integrity, config)
            if enforce_topology
            else True
        ),
        ...
        "off_center_gradients_every_update": (
            len(history) == maximum
            and all(
                set(value.get("off_center_gradient", {}))
                == {"null", "treatment"}
                and all(
                    ...
                    for row in value["off_center_gradient"].values()
                )
                for item, value in enumerate(history)
            )
            if enforce_topology
        ...
        "first_update_off_center_gradient_hashes_distinct": (
            set(first.get("off_center_gradient", {})) == {"null", "treatment"}
            and first["off_center_gradient"]["null"].get("sha256")
            != first["off_center_gradient"]["treatment"].get("sha256")
            if enforce_topology
            else True
        ),
        "cumulative_off_center_gradient_coverage_complete_by_update2": (
            _early_topology_coverage_valid(history, len(history), config)
            if enforce_topology
            else True
        ),
        "update2_off_center_weights_finite_nonzero_and_distinct": (
            all(
                row.get("finite") is True
                and row.get("count") == OFF_CENTER_COEFFICIENT_COUNT
                and row.get("nonzero_count") == OFF_CENTER_COEFFICIENT_COUNT
                for row in (
                    history[1].get("off_center_weight_after", {})
                    if len(history) >= 2
                    else {}
                ).values()
            )
            and len(history) >= 2
            and set(history[1].get("off_center_weight_after", {}))
            == {"null", "treatment"}
            and history[1]["off_center_weight_after"]["null"].get("sha256")
            != history[1]["off_center_weight_after"]["treatment"].get("sha256")
            if enforce_topology
            else True
        ),
        "final_off_center_weights_finite_nonzero_and_distinct": (
            all(
                row.get("finite") is True
                and row.get("count") == OFF_CENTER_COEFFICIENT_COUNT
                and row.get("nonzero_count") == OFF_CENTER_COEFFICIENT_COUNT
                for row in final.get("off_center_weight_after", {}).values()
            )
            and set(final.get("off_center_weight_after", {}))
            == {"null", "treatment"}
            and final["off_center_weight_after"]["null"].get("sha256")
            != final["off_center_weight_after"]["treatment"].get("sha256")
            and final_model_states_differ
            if enforce_topology
            else True
        ),
...
def _final_payload(
    ...
    integrity = _training_integrity(
        ...
        topology_integrity,
        final_resume_equal,
        current_hashes["null"] != current_hashes["treatment"],
    )
...
def _verify_joint_final(
    ...
    topology_integrity = final.get("initial_topology_integrity", {})
    topology_commitment = topology_integrity.get("integrity_sha256")
    topology_payload = {
        name: value
        for name, value in topology_integrity.items()
        if name != "integrity_sha256"
    }
    ...
    if (
        ...
        or final.get("determinism") != expected_determinism
        or final.get("permutation_contract_sha256") != permutation_sha256
        ...
        or lineage.get("consumed_or_forbidden_prior_seeds")
        != config["data"]["consumed_or_forbidden_prior_seeds"]
        or lineage.get("permutation_contract_sha256") != permutation_sha256
        or (
            config["integrity"].get("enforce_topology_gradient_audit", True)
            and (
                topology_commitment != _canonical_sha256(topology_payload)
                or not _topology_integrity_valid(topology_integrity, config)
            )
        )
        ...
        or (
            config["integrity"].get("enforce_topology_gradient_audit", True)
            and (
                not _initialization_receipt_valid(initialization, config)
            )
        )
        or initialization.get("determinism") != expected_determinism
    ...
    if resume_path is not None:
        ...
        if (
            ...
            or resume.get("determinism") != expected_determinism
            or resume.get("permutation_contract_sha256") != permutation_sha256
            ...
            or resume.get("initial_topology_integrity")
            != topology_integrity
    ...
    expected_integrity = _training_integrity(
        ...
        topology_integrity,
        final_resume_equal,
        final["model_state_sha256"]["null"]
        != final["model_state_sha256"]["treatment"],
    )
...
def _freeze_payload(
    ...
    return {
        ...
        "family_self_sha256": config["family_self_sha256"],
        "source_sha256": frozen_sources,
        "determinism": final["determinism"],
        "permutation_contract_sha256": final["permutation_contract_sha256"],
        "config_contract_sha256": config["contract_sha256"],
        "config_file_sha256": config["config_file_sha256"],
...
def verified_qualification_capability(
    ...
    return {
        ...
        "determinism": expected["determinism"],
        "permutation_contract_sha256": expected["permutation_contract_sha256"],
...
def qualification_manifests(
    ...
    if (
        ...
        or capability.get("permutation_contract_sha256")
        != config["family"]["permutation_contract"]["permutation_contract_sha256"]
    ):
...
def _trajectory_receipt(
    ...
    frozen_sources = _assert_frozen_files(config)
    payload = {
        "format": "independent-atlas-pair-topology-paired-trajectory-v1",
        "purpose": PURPOSE,
        "family_self_sha256": config["family_self_sha256"],
        "source_sha256": frozen_sources,
        "determinism": {
...
def _qualification_result_semantics_valid(result: dict, config: dict) -> bool:
    ...
    return (
        ...
        and result.get("lineage", {}).get("permutation_contract_sha256")
        == config["family"]["permutation_contract"]["permutation_contract_sha256"]
        and result.get("lineage", {}).get("determinism")
        == {
            "deterministic_algorithms": True,
            "cudnn_deterministic": True,
            "cudnn_benchmark": False,
...
def _existing_receipt_is_valid(path: Path, config: dict, final_path: Path) -> bool:
    ...
    if (
        ...
        or receipt.get("permutation_contract_sha256")
        != config["family"]["permutation_contract"]["permutation_contract_sha256"]
...
def run(config_path: str | Path) -> Path:
    ...
    _assert_no_stale_temporaries(run_folder)
    generator = SyntheticRegistrationGenerator(
    ...
    initialization = bind_optimizer_scaler_initialization(
    ...
    setup = {
        "format": "independent-atlas-pair-topology-paired-setup-v1",
        "purpose": PURPOSE,
        "role": ROLE,
        "family_self_sha256": config["family_self_sha256"],
        "config_contract_sha256": config["contract_sha256"],
    ...
    if resume_path.exists():
        update, history, development, topology_integrity = _load_joint_resume(
            resume_path,
            config,
            lineage,
            models,
    ...
    while update < config["training"]["max_updates"]:
        ...
        if topology_integrity is None:
            topology_integrity = initial_topology_integrity(models, batch)
        ...
        if update == config["integrity"]["early_gradient_coverage_window_updates"] and not _early_topology_coverage_valid(
            history, update, config
        ):
            raise RuntimeError(
                "cumulative off-centre gradient coverage failed by update 2"
            )
        ...
        if (
            ...
            print(
                json.dumps(
                    {
                        "event": "paired-checkpoint-persisted",
                        "update": update,
                        "file": resume_path.name,
                        "file_sha256": _sha256(resume_path),
                    },
                    sort_keys=True,
                    separators=(",", ":"),
                ),
    ...
    if topology_integrity is None or not resume_path.is_file():
    ...
    if final_path.exists():
        ...
        print(
            json.dumps(
                {
                    "event": "paired-final-checkpoint-persisted",
                    "update": config["training"]["max_updates"],
    ...
    for manifest in manifests:
        ...
        manifest_path = write_manifest_receipt(
            ...
            config,
            {
                ...
                "permutation_contract_sha256": capability[
                    "permutation_contract_sha256"
                ],
            },
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(arguments) != 1:
        raise SystemExit(
            "usage: python -m training.run_independent_atlas_pair_topology CONFIG"
        )
