...
AtlasPose was trained with arbitrary in-plane rotation and 0.5–1.5× scale augmentation. At inference, the trusted brain mask supplies roll and scale canonicalization, and an auxiliary orientation output resolves the remaining 180° ambiguity. That orientation is carried into the slice-to-atlas affine, so rotating or flipping the source image does not silently rotate the displayed overlay. DeepSlice receives the corresponding preprocessed crop. Horizontal and vertical flip controls are applied to the slice and its trusted points before inference.
...
Automatic registration is deliberately two-stage. Pose search first freezes AP, L-R/D-V tilt, and the surface-calibrated affine transform. A residual model may then apply a bounded in-plane diffeomorphic refinement to internal anatomy on that exact atlas plane. The refinement cannot repair a wrong AP/tilt estimate, folded or torn tissue, or missing anatomy; rejected pairs remain affine-only. Native MIND and surface-overlap checks are secondary surrogates, so the feature remains experimental and source-disabled until a frozen animal-disjoint internal-landmark benchmark independently passes and the approved model/evidence hashes are pinned in source. Review every accepted result using the atlas blend and 3D planes.
...
Optional surgical constraints are stored independently for each probe. Enter the planned bregma-centred AP/ML coordinate and its uncertainty radius, plus an attack angle and tolerance; the angle convention is **0 degrees horizontal and 90 degrees vertical**. The uncertainty circle is projected onto the Allen Isocortex surface identified through `query.csv` and shown in the active probe's color; target areas without a valid cortical entry are rejected, and boundary portions outside dorsal Isocortex are omitted with a UI warning. The selected probe's 10 mm physical shank length is always enforced, and an optional planned depth can tighten that bound. When enabled and supported by at least two marked trajectory observations, these physical constraints help select among otherwise plausible slice alignments. When disabled, automatic alignment follows the original image-only path exactly while an over-length displayed/mapped trajectory is rejected explicitly.

The dedicated desktop source folder and bundled desktop build include checksum-locked ONNX conversions of the official DeepSlice 1.2.8 primary and secondary mouse models. AtlasPose remains disabled for release until a frozen candidate passes its one-shot sealed gate and its model, metadata, and release-evidence hashes are pinned in application source. ONNX Runtime uses the first supported GPU provider by default and falls back to CPU; export checks compare CPU with every available application accelerator provider. DeepSlice should be cited as Carey et al., *Nature Communications* 14, 5884 (2023). Its repository LICENSE is included in `licenses/DeepSlice-LICENSE.txt`; current repository and PyPI license metadata are contradictory, so maintainer clarification is required before redistributing it in a closed-source bundle. AtlasPose uses Allen-derived data and permissively licensed pretrained timm weights, but this project does not grant redistribution or commercial-use clearance for Allen data or derived weights.

Generated executables, bundled runtimes, atlas data, the two approximately 93 MB DeepSlice ONNX binaries, and AtlasPose ONNX candidates are intentionally excluded from Git. Their small metadata files retain model hashes and validation results. Local builds require the validated DeepSlice binaries; `TrajectoryTracker.spec` includes AtlasPose only when its source-approved model, metadata, raw sealed predictions, metrics, presealed commitment, exclusive claim, completion receipt, and release report all pass the runtime hash-chain verification. The desktop shortcut targets the bundled executable directly, and the application resolves the atlas from this dedicated desktop folder without a wrapper process.
