...
Automatic registration is deliberately two-stage. Pose search first freezes AP, L-R/D-V tilt, and the surface-calibrated affine transform. A separately validated residual model may then apply a bounded in-plane diffeomorphic refinement to internal anatomy on that exact atlas plane. The refinement cannot repair a wrong AP/tilt estimate, folded or torn tissue, or missing anatomy; rejected pairs remain affine-only. The option is disabled unless its ONNX model and locked release evidence match hashes approved in source. Review every accepted result using the atlas blend and 3D planes.
