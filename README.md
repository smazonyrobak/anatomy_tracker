...
Automatic registration is deliberately two-stage. **Auto-align** first freezes AP, L-R/D-V tilt, and the surface-calibrated affine transform. After reviewing that pose, **Fit current slice to atlas (nonlinear)** may apply a bounded in-plane diffeomorphic refinement to internal anatomy on that exact atlas plane. The fit has its own non-modal progress/cancel flow and cannot alter AP or either tilt. It cannot repair a wrong pose, folded or torn tissue, or missing anatomy; rejected pairs retain their affine alignment. Native MIND and surface-overlap checks are secondary surrogates, so the button remains unavailable until a frozen animal-disjoint internal-landmark benchmark independently passes and the approved model/evidence hashes are pinned in source. Review every accepted result using the atlas blend and 3D planes.
