...
The dedicated desktop source folder and bundled desktop build include checksum-locked ONNX conversions of the official DeepSlice 1.2.8 primary and secondary mouse models plus the selected AtlasPose ConvNeXt-Tiny model for offline inference. ONNX Runtime DirectML uses the first available GPU by default and falls back to CPU if DirectML is unavailable; export checks compare CPU and DirectML outputs. DeepSlice should be cited as Carey et al., *Nature Communications* 14, 5884 (2023). Its repository LICENSE is included in `licenses/DeepSlice-LICENSE.txt`; current repository and PyPI license metadata are contradictory, so maintainer clarification is required before redistributing it in a closed-source bundle. AtlasPose was trained from Allen atlas-derived synthetic sections; this project does not grant redistribution or commercial-use clearance for the Allen source data or derived model, so review the applicable Allen Institute terms before shipping it.

Generated executables, bundled runtimes, atlas data, the two approximately 93 MB DeepSlice ONNX binaries, and the approximately 112 MB AtlasPose ONNX binary are intentionally excluded from Git. Their small metadata files retain model hashes and validation results. Local builds require the validated binaries to be present; `TrajectoryTracker.spec` bundles all three models into the deployed application. The desktop shortcut targets the bundled executable directly, and the application resolves the atlas from this dedicated desktop folder without a wrapper process.

## AtlasPose validation status

ConvNeXt-Tiny won the controlled 30k synthetic comparison and, after 100k-image training with validation-based checkpoint selection, reached sealed synthetic-test MAE of 58.72 µm AP, 0.934° L–R, and 1.052° D–V. On 148 untouched real DeepSlice benchmark sections inside the +500 to −4500 µm AP domain, AtlasPose reached 245.20 µm, 1.639°, and 3.996° MAE versus 174.26 µm, 1.463°, and 1.268° for the published DeepSlice outputs. It is therefore an experimental independent predictor, not a demonstrated DeepSlice replacement. The optional vote defaults to 20% AtlasPose and 80% DeepSlice; on that benchmark it reached 140.61 µm, 1.20°, and 1.63°. These aggregate results do not remove the requirement to review individual overlays. Full architecture, augmentation, staged, percentile, and benchmark details are in `training/README.md`.
