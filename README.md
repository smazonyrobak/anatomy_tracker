...
The workflow is deliberately two-stage. **Automatic section matching** estimates AP and L-R/D-V cutting tilt. **Apply landmark warp** then fits an in-plane thin-plate spline through corresponding anatomical landmarks on that fixed atlas plane. The same nonlinear coordinate map renders the histology overlay and translates every marked electrode point before 3-D trajectory fitting and anatomical export. Editing landmarks invalidates only this coordinate map, not the automatic AP/tilt result; applying the warp again deterministically recomputes all probe coordinates.
...
After applying a landmark warp or automatically matching a slice, use the **Slice — Atlas** blend slider directly below the atlas comparison image to inspect either image alone or any intermediate opacity.
...
After **Map channels/units** succeeds, the app opens an Allen-colored physical probe view. Every recording site is shown at its real shank position, contiguous depth bands identify the structures traversed, hovering reports the channel and assigned region, and the adjacent table reports channel/unit counts per structure. **View mapped anatomy** reopens the saved result for the active probe at any time.
