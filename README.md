...
Optional surgical constraints are stored independently for each probe and apply only when **Auto-align current** or **Auto-align all** is run after editing them; they never alter an existing alignment or the later displayed regression. Enter the planned bregma-centred AP/ML coordinate and its uncertainty radius, plus an attack angle and tolerance; the angle convention is **0 degrees horizontal and 90 degrees vertical**. The uncertainty circle is projected onto the Allen Isocortex surface identified through `query.csv` and shown in the active probe's color. Candidate poses are accepted only when the ordinary post-alignment trajectory shown by the UI itself lies inside the AP/ML disk, attack-angle interval, and depth/physical-shank bounds. Invalid cortical entries are rejected. Surface samples are cached once per run, so feasibility restricts rather than expands the atlas search. When disabled, automatic alignment follows the image-only path exactly.
